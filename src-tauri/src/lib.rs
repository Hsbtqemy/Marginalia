...
use docx_rs::{
    Comment, Docx, Document, Footnote, LineSpacing, LineSpacingType, PageMargin, PageSize,
    Paragraph, Run, RunFonts, SectionProperty,
};
...
fn export_docx(payload: ExportDocxPayload) -> Result<String, String> {
    ...
    for block in &manuscript_blocks {
        let mut paragraph = paragraph_from_block(block, &payload.preset);
        ...
        if is_working_profile {
            if let Some(annotations) = linked_left_by_block.get(&block.block_id) {
                for annotation in annotations {
                    ...
                    let comment = Comment::new(comment_id)
                        .author("Marginalia")
                        .add_paragraph(Paragraph::new().add_run(run_from_text(&annotation.text, &payload.preset, 1.0, false, false)));

                    paragraph = paragraph.add_comment_start(comment);
                    paragraph = paragraph.add_run(run_from_text("◊", &payload.preset, 1.0, false, false));
            ...
            if let Some(footnotes) = linked_right_by_block.get(&block.block_id) {
                for footnote_block in footnotes {
                    ...
                    let footnote = Footnote::new().add_content(
                        Paragraph::new()
                            .add_run(run_from_text(&footnote_block.text, &payload.preset, 1.0, false, false)),
                    );
    ...
    if is_working_profile && payload.include_unlinked_left_annex && !unlinked_left.is_empty() {
        let annex_heading = Paragraph::new()
            ...
            .add_run(run_from_text("Chutier", &payload.preset, payload.preset.heading_scale.h1, true, false));
...
fn read_node_state_string(node: &Value, key: &str) -> Option<String> {
    node.get("$")
        ...
        .or_else(|| node.get(key).and_then(Value::as_str).map(ToString::to_string))
}
...
fn paragraph_from_block(block: &ManuscriptBlock, preset: &ExportPresetPayload) -> Paragraph {
    ...
    match block.kind {
        ManuscriptKind::Paragraph => paragraph
            .add_run(run_from_text(&block.text, preset, 1.0, false, false)),
        ManuscriptKind::Heading(level) => {
            ...
            paragraph.add_run(run_from_text(&block.text, preset, scale, true, false))
        }
        ManuscriptKind::Quote => {
            paragraph = paragraph.style("Quote");
            paragraph.add_run(run_from_text(&block.text, preset, 1.0, false, true))
        }
